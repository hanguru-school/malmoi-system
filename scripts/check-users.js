...
async function checkUsers() {
  try {
    ...
    // 모든 사용자 조회
    const usersResult = await pool.query('SELECT * FROM users ORDER BY "createdAt" DESC');
    ...
    for (const user of usersResult.rows) {
      ...
      if (user.role === 'STUDENT') {
        const studentResult = await pool.query('SELECT * FROM students WHERE "userId" = $1', [user.id]);
        ...
        const adminResult = await pool.query('SELECT * FROM admins WHERE "userId" = $1', [user.id]);
