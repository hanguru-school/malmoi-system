...
export default function GrammarClassificationPage() {
  ...
  const [activeTab, setActiveTab] = useState<'completed' | 'frequent' | 'unexplained'>('unexplained');
  const [searchTerm, setSearchTerm] = useState('');
  const [selectedType, setSelectedType] = useState<'all' | 'grammar' | 'vocabulary' | 'expression'>('all');
      ...
      const mockLessonNotes: LessonNote[] = [
        {
          ...
          studentName: '김학생',
          date: '2024-01-15',
          content: '안녕하세요. 만나서 반갑습니다. 저는 한국어를 배우고 있습니다.',
  ...
  const handleCategoryChange = (itemId: string, newCategory: 'completed' | 'frequent' | 'unexplained') => {
    setGrammarItems(prev => prev.map(item => 
      item.id === itemId 
        ? { ...item, category: newCategory, explanationCount: newCategory === 'completed' ? item.explanationCount + 1 : item.explanationCount }
        : item
    ));
  };
  ...
  const getFilteredItems = () => {
    ...
    if (searchTerm) {
      filtered = filtered.filter(item => 
        ...
        item.relatedExamples.some(example => example.toLowerCase().includes(searchTerm.toLowerCase()))
      );
    }
  ...
  const getCategoryStats = () => {
    const completed = grammarItems.filter(item => item.category === 'completed').length;
    const frequent = grammarItems.filter(item => item.category === 'frequent').length;
    const unexplained = grammarItems.filter(item => item.category === 'unexplained').length;
  ...
  const getItemIcon = (type: string) => {
    switch (type) {
      case 'grammar': return <Hash className="w-4 h-4" />;
      case 'vocabulary': return <BookOpen className="w-4 h-4" />;
      case 'expression': return <MessageSquare className="w-4 h-4" />;
  ...
  const getCategoryIcon = (category: string) => {
    switch (category) {
      ...
      case 'frequent': return <TrendingUp className="w-4 h-4 text-blue-600" />;
      case 'unexplained': return <AlertTriangle className="w-4 h-4 text-orange-600" />;
      ...
      <div className="bg-white shadow-sm border-b">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center py-6">
            <div>
              <h1 className="text-3xl font-bold text-gray-900">문법/단어/용법 분류 시스템</h1>
              <p className="mt-2 text-gray-600">자동 추출 및 분류 기반 학습 흐름 관리</p>
            </div>
                  ...
                  >
                    <div className="flex items-center justify-between">
                      ...
                      <div className="text-right">
                        ...
                        <div className="text-sm font-medium text-blue-600">{student.progress}%</div>
                      </div>
                    </div>
                    <div className="mt-2 space-y-1">
                      <div className="flex justify-between text-xs">
                        <span>문법</span>
                        <span className="text-green-600">{student.grammarProgress}%</span>
                      </div>
                      <div className="flex justify-between text-xs">
                        <span>단어</span>
                        <span className="text-blue-600">{student.vocabularyProgress}%</span>
                      </div>
                      <div className="flex justify-between text-xs">
                        <span>표현</span>
                        <span className="text-purple-600">{student.expressionProgress}%</span>
                      </div>
                    </div>
                  </div>
                ))}
          ...
          <div className="lg:col-span-3">
            {selectedStudent ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  <div className="bg-white rounded-lg shadow-sm border p-6">
                    <div className="flex items-center">
                      ...
                      <div className="ml-4">
                        <p className="text-sm font-medium text-gray-600">설명 완료</p>
                  ...
                  <div className="bg-white rounded-lg shadow-sm border p-6">
                    <div className="flex items-center">
                      ...
                      <div className="ml-4">
                        <p className="text-sm font-medium text-gray-600">자주 언급됨</p>
                  ...
                  <div className="bg-white rounded-lg shadow-sm border p-6">
                    <div className="flex items-center">
                      ...
                      <div className="ml-4">
                        <p className="text-sm font-medium text-gray-600">미설명</p>
                        <p className="text-2xl font-bold text-gray-900">{stats.unexplained}</p>
                      </div>
                    </div>
                  </div>
                </div>
                  ...
                  <div className="p-6">
                    {filteredItems.length === 0 ? (
                      <div className="text-center py-12">
                        ...
                        <p className="text-gray-500">해당 조건의 항목이 없습니다.</p>
                      </div>
                    ) : (
                        ...
                        {filteredItems.map(item => (
                          <div
                            ...
                            className={`p-4 border rounded-lg cursor-pointer transition-all hover:shadow-md ${
                              item.isHighlighted ? 'bg-yellow-50 border-yellow-200' : 'bg-white border-gray-200'
                            }`}
                          >
                            <div className="flex items-start justify-between">
                              <div className="flex-1">
                                ...
                                <div className="flex items-center space-x-4 text-sm text-gray-600 mb-2">
                                  <span>레벨 {item.level}-{item.subLevel}</span>
              ...
              <div className="bg-white rounded-lg shadow-sm border p-12 text-center">
                ...
                <p className="text-gray-500">왼쪽에서 학생을 선택하면 문법 분류 현황을 확인할 수 있습니다.</p>
              </div>
            )}
          </div>
        </div>
            ...
            <div className="p-6 border-b border-gray-200">
              <div className="flex items-center justify-between">
                <div className="flex items-center space-x-3">
                  ...
                  <h2 className="text-xl font-bold text-gray-900">{selectedItem.content}</h2>
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div className="bg-gray-50 p-4 rounded-lg">
                  ...
                  <p className="font-medium">{selectedItem.level}-{selectedItem.subLevel}</p>
                </div>
                <div className="bg-gray-50 p-4 rounded-lg">
                ...
                <div className="bg-gray-50 p-4 rounded-lg">
                  ...
                  <p className="font-medium">{selectedItem.explanationCount}회</p>
                </div>
                <div className="bg-gray-50 p-4 rounded-lg">
                  <p className="text-sm text-gray-600">오답 횟수</p>
                  <p className="font-medium">{selectedItem.homeworkMistakes}회</p>
                </div>
              </div>

              {/* Related Examples */}
              <div>
                <h3 className="text-lg font-semibold text-gray-900 mb-3">관련 예문</h3>
              ...
              {selectedItem.explanationHistory.length > 0 && (
                <div>
                  <h3 className="text-lg font-semibold text-gray-900 mb-3">설명 이력</h3>
                  <div className="space-y-3">
                    {selectedItem.explanationHistory.map((history, index) => (
                      <div key={index} className="bg-green-50 p-4 rounded-lg">
                        <div className="flex items-center justify-between mb-2">
                          ...
                          <span className="text-xs text-gray-500">레슨 {history.lessonId}</span>
                        </div>
