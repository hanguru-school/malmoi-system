...
export class ExternalIntegrationManager {
  ...
  // SimplyBook 연동
  async syncWithSimplyBook(bookingData: Record<string, unknown>): Promise<boolean> {
    if (!this.config.simplyBook.enabled) return false;

    try {
      const response = await fetch(`${this.config.simplyBook.apiUrl}/bookings`, {
        method: 'POST',
        headers: {
  ...
  // Square 결제 처리
  async processSquarePayment(paymentData: Record<string, unknown>): Promise<any> {
  ...
  // Google Calendar 연동
  async addToGoogleCalendar(eventData: Record<string, unknown>): Promise<string | null> {
  ...
  // UID 태깅 이벤트 처리
  async processUIDTagEvent(uid: string, location: string): Promise<UIDTagEvent | null> {
  ...
  private async getUserByUID(uid: string): Promise<any> {
    try {
      const response = await fetch(`${this.config.uidReader.apiEndpoint}/users/${uid}`, {
        headers: {
  ...
  // 이벤트 큐에 추가
  addToEventQueue(event: Omit<IntegrationEvent, 'id' | 'timestamp' | 'status' | 'retryCount'>): void {
    ...
    for (const event of pendingEvents) {
      try {
        ...
        if (event.retryCount < 3) {
          ...
          }, Math.pow(2, event.retryCount) * 1000); // 지수 백오프
        }
      }
    }
  }
  ...
  private async handleNotificationEvent(event: IntegrationEvent): Promise<void> {
...
export const externalIntegrationManager = ExternalIntegrationManager.getInstance(); 