        ...
        try {
          ...
          for (const port of ports) {
            ...
            if (port.getInfo) {
              ...
              if (info.usbVendorId === RC_S380_CONFIG.vendorId && 
    ...
    try {
      ...
      if ('serial' in navigator) {
        ...
        try {
          ...
          let rcS380Port = ports.find((port: any) => {
            if (port.getInfo) {
              const info = port.getInfo();
              return info.usbVendorId === RC_S380_CONFIG.vendorId && 
                     info.usbProductId === RC_S380_CONFIG.productId;
            }
            return false;
          });
      ...
      if ('hid' in navigator) {
        ...
        try {
          ...
          const rcS380Device = devices.find((device: any) => 
      ...
      // 5. 대체 모드 (개발/테스트용)
      console.log('실제 하드웨어 리더를 찾을 수 없습니다. 대체 모드로 전환합니다.');
      ...
      if (this.port && this.port.readable) {
        ...
        try {
          ...
          while (true) {
            ...
            const uidMatch = data.match(/UID[:\s]*([A-F0-9]+)/i) || 
      ...
      if (this.port && this.port.sendReport) {
        ...
        return new Promise((resolve, reject) => {
          ...
          this.port.addEventListener('inputreport', (event: any) => {
            ...
            const uid = Array.from(data.slice(0, 8)).map(b => b.toString(16).padStart(2, '0')).join('').toUpperCase();
