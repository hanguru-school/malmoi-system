...
async function checkReservations() {
  try {
    ...
    // 예약 데이터 조회
    const reservationsResult = await pool.query('SELECT * FROM reservations ORDER BY "createdAt" DESC LIMIT 10');
    ...
    if (reservationsResult.rows.length > 0) {
      ...
      reservationsResult.rows.forEach((reservation, index) => {
        ...
        console.log(`   시간: ${reservation.startTime} ~ ${reservation.endTime}`);
