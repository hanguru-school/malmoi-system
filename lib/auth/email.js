...
function resolveDefaultPortalUrl() {
  const fromEnv = process.env.MAIL_LINK_BASE_URL || process.env.APP_BASE_URL || process.env.NEXTAUTH_URL;
  const cleaned = String(fromEnv || "").trim().replace(/\/$/, "");
  return cleaned || "https://portal.hanguru.blog";
}

const PORTAL_URL = resolveDefaultPortalUrl();
...
function buildAdminPasswordResetMail({ resetUrl, expiresAt }) {
  return buildStudentMailTemplate({
    subject: `【MalMoi韓国語教室】管理者パスワード再設定のご案内`,
    headline: "管理者アカウントのパスワードを再設定してください",
    introLines: [
      "管理者向けパスワード再設定のリクエストを受け付けました。",
    ...
    expiresAt,
    subLines: [
      "このリンクは有効時間が限られており、1回使うと無効になります。",
      "心当たりがない場合は、このメールを破棄してください。",
    ],
  });
}
...
export async function sendAdminPasswordResetMail({ toEmail, resetUrl, expiresAt }) {
  const payload = buildAdminPasswordResetMail({ resetUrl, expiresAt });
  return deliverMail({
    toEmail,
    payload,
    logLabel: "admin password reset link",
    linkForLog: resetUrl,
    mailType: "admin_password_reset",
    templateName: "admin_password_reset_mail",
    meta: { recipientRole: "admin" },
  });
}
