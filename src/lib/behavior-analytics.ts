...
export interface BehaviorTrigger {
  type: 'no_booking_7days' | 'no_note_view' | 'no_attendance' | 'low_hours' | 'before_class';
...
export class BehaviorAnalyticsManager {
  ...
  private initializeDefaultRules(): void {
    this.reminderRules = [
      ...
      },
      {
        ...
        message: '今月の学習時間が180分に満たないようです。目標達成のためにもう一度予約してみましょう！',
        channels: ['line', 'email'],
        timing: 'monthly',
        enabled: true,
        priority: 'high'
  ...
  // 사용자 행동 업데이트
  async updateUserBehavior(userId: string, userType: 'student' | 'teacher' | 'admin', updates: Partial<UserBehavior>): Promise<void> {
    ...
    const updatedBehavior = { ...currentBehavior, ...updates, lastActivityDate: new Date() };
  ...
  private async analyzeBehaviorPattern(userId: string): Promise<void> {
    ...
    if (behavior.lastBookingDate) {
      const daysSinceLastBooking = Math.floor((Date.now() - behavior.lastBookingDate.getTime()) / (1000 * 60 * 60 * 24));
  ...
  // 조건 평가
  private async evaluateCondition(condition: string, behavior: UserBehavior): Promise<boolean> {
    switch (condition) {
      case 'lastBookingDate < 7 days ago':
        if (!behavior.lastBookingDate) return true;
        const daysSinceBooking = Math.floor((Date.now() - behavior.lastBookingDate.getTime()) / (1000 * 60 * 60 * 24));
        return daysSinceBooking >= 7;

      case 'lastNoteViewDate < lastAttendanceDate':
        if (!behavior.lastAttendanceDate || !behavior.lastNoteViewDate) return false;
  ...
  // 리마인드 생성
  private async createReminder(userId: string, userType: 'student' | 'teacher' | 'admin', rule: ReminderRule): Promise<void> {
    // 중복 리마인드 방지
    const existingReminder = this.reminderQueue.find(
      r => r.userId === userId && r.ruleId === rule.id && r.status === 'pending'
    );
  ...
  async processReminderQueue(): Promise<void> {
    ...
    for (const reminder of pendingReminders) {
      try {
        ...
        console.log(`리마인드 발송 완료: ${reminder.userId} - ${reminder.ruleId}`);
      } catch (error) {
        ...
        if (reminder.retryCount < 3) {
          ...
          reminder.scheduledAt = new Date(Date.now() + Math.pow(2, reminder.retryCount) * 60 * 1000);
        }
      }
    }
  ...
  // 리마인드 발송
  private async sendReminder(reminder: ReminderMessage, channel: string): Promise<void> {
    switch (channel) {
      case 'line':
        await externalIntegrationManager.sendLineMessage(reminder.userId, reminder.message);
        break;
      case 'email':
  ...
  async getAllBehaviorStats(): Promise<{
    ...
    const activeUsers = behaviors.filter(b => b.bookingPattern !== 'inactive').length;
    const averageAttendanceRate = behaviors.reduce((sum, b) => sum + b.attendanceRate, 0) / totalUsers || 0;
    const averageMonthlyHours = behaviors.reduce((sum, b) => sum + b.monthlyClassHours, 0) / totalUsers || 0;
    const inactiveUsers = behaviors.filter(b => b.bookingPattern === 'inactive').length;
  ...
  getReminderHistory(userId: string): ReminderMessage[] {
    return this.reminderQueue.filter(r => r.userId === userId && r.status === 'sent');
  }
