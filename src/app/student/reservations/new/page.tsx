...
export default function NewReservationPage() {
  ...
  const [step, setStep] = useState<'course' | 'datetime' | 'confirm' | 'complete'>('course');
      ...
      <div className="max-w-4xl mx-auto">
                ...
                <div key={stepItem.id} className="flex items-center">
                  <div className={`flex items-center gap-2 px-4 py-2 rounded-lg ${
                    step === stepItem.id 
                      ...
                      : index < ['course', 'datetime', 'confirm'].indexOf(step)
        ...
        {step === 'course' && (
          <div className="bg-white rounded-xl shadow-lg p-6">
            <h2 className="text-xl font-semibold text-gray-900 mb-6">수업 코스 선택</h2>
                ...
                >
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                      course.level === '초급' ? 'bg-green-100 text-green-700' :
                  ...
                  <div className="space-y-2 mb-4">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm text-gray-600">{course.duration}분</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <MapPin className="w-4 h-4 text-gray-500" />
                      <span className="text-sm text-gray-600">
                        {course.type === 'online' ? '온라인' : 
                         course.type === 'offline' ? '대면' : '온라인/대면'}
                      </span>
                    </div>
                    {course.teacher && (
                      <div className="flex items-center gap-2">
                        ...
                        <div className="flex items-center gap-1">
                          ...
                          <span className="text-xs text-gray-600">{course.teacher.rating}</span>
                        </div>
                      </div>
                    )}
                  </div>
        ...
        {step === 'datetime' && selectedCourse && (
          <div className="bg-white rounded-xl shadow-lg p-6">
              ...
              <div className="grid grid-cols-7 gap-1">
                ...
                {days.map((day, index) => {
                  ...
                  return (
                    <button
                      key={index}
                      onClick={() => !isPast && handleDateSelect(day.toISOString().split('T')[0])}
                      disabled={isPast}
            ...
            {selectedDate && (
              <div>
                <h3 className="text-lg font-medium text-gray-900 mb-4">시간대 선택</h3>
                    ...
                    >
                      <div className="flex items-center justify-between">
                        ...
                        <div className={`w-4 h-4 rounded-full border-2 ${
                          timeSlot.available ? 'border-blue-500' : 'border-gray-300'
                        }`}>
                          {timeSlot.available && <div className="w-2 h-2 bg-blue-500 rounded-full m-0.5"></div>}
                        </div>
                      </div>
                    </button>
                  ))}
        ...
        {step === 'confirm' && selectedCourse && selectedTimeSlot && (
          <div className="bg-white rounded-xl shadow-lg p-6">
            <h2 className="text-xl font-semibold text-gray-900 mb-6">예약 정보 확인</h2>
            ...
            <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="font-medium text-gray-900">{selectedTimeSlot.teacherName}</div>
                    </div>
                  </div>
                </div>
              </div>
                ...
                <div className="bg-green-50 rounded-lg p-6">
                  ...
                  <div className="flex items-center gap-3">
                    <div className="flex-1">
                      <div className="text-sm text-gray-600 mb-1">수업 링크</div>
                      <div className="font-medium text-gray-900">https://zoom.us/j/123456789</div>
                    </div>
                ...
                <div className="space-y-3">
                  <label className="flex items-start gap-3 cursor-pointer">
                    <input
                      ...
                      onChange={(e) => setReservationData(prev => ({ ...prev, agreedToTerms: e.target.checked }))}
                      className="mt-1 w-4 h-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500"
                    />
                    <div className="text-sm text-gray-700">
                      <div className="font-medium mb-1">예약 규정에 동의합니다</div>
                      <div className="text-gray-600">
                        • 수업 24시간 전까지 취소 가능<br/>
                        • 수업 시작 10분 전까지 입장<br/>
                        • 개인정보 수집 및 이용에 동의
                      </div>
                    </div>
              ...
              {/* 메모 입력 */}
              <div>
                <h3 className="text-lg font-medium text-gray-900 mb-4">메모 (선택사항)</h3>
                <textarea
