  ...
  try {
    ...
    if (error) {
      ...
      return NextResponse.redirect(
        new URL(
          `/auth/login?error=${error}&error_description=${errorDescription}`,
          request.url,
        ),
      );
    }
    ...
    if ("code" in result) {
      ...
      return NextResponse.redirect(
        new URL(
          `/auth/login?error=${result.code}&error_description=${result.message}`,
          request.url,
        ),
      );
    }
    ...
    return NextResponse.redirect(
      new URL(
        `/auth/login?error=callback_error&error_description=${encodeURIComponent(error.message)}`,
        request.url,
      ),
    );
  }
