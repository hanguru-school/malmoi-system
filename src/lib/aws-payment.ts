...
// 결제 방법 타입
export type PaymentMethod = 'card' | 'bank_transfer' | 'virtual_account' | 'phone';
...
export class PaymentService {
  ...
  async requestTossPayment(paymentData: {
    ...
    try {
      ...
      // 토스페이먼츠 API 호출
      const response = await fetch(`${paymentConfig.tossPayments.apiUrl}/v1/payments`, {
        method: 'POST',
        headers: {
  ...
  // 토스페이먼츠 결제 승인
  async confirmTossPayment(paymentKey: string, orderId: string, amount: number) {
    try {
      const response = await fetch(`${paymentConfig.tossPayments.apiUrl}/v1/payments/${paymentKey}`, {
        method: 'POST',
        headers: {
          'Authorization': `Basic ${Buffer.from(paymentConfig.tossPayments.secretKey + ':').toString('base64')}`,
          'Content-Type': 'application/json'
  ...
  async cancelPayment(paymentId: string, reason: string) {
    try {
      const response = await fetch(`${paymentConfig.tossPayments.apiUrl}/v1/payments/${paymentId}/cancel`, {
        method: 'POST',
        headers: {
          'Authorization': `Basic ${Buffer.from(paymentConfig.tossPayments.secretKey + ':').toString('base64')}`,
          'Content-Type': 'application/json'
