...
interface Reminder {
  id: string;
  type: 'lesson-reminder' | 'monthly-summary' | 'tagging-warning' | 'encouragement' | 'next-booking' | 'no-booking' | 'mid-month' | 'next-month';
  title: string;
  message: string;
...
export default function ReminderSystemPage() {
  ...
  const [filterType, setFilterType] = useState<'all' | 'lesson-reminder' | 'monthly-summary' | 'tagging-warning' | 'encouragement' | 'next-booking' | 'no-booking' | 'mid-month' | 'next-month'>('all');

  const reminderTypes = [
    { id: 'lesson-reminder', name: '수업 전일 리마인드', icon: <Bell className="w-4 h-4" /> },
    { id: 'monthly-summary', name: '월초 예약 요약', icon: <Calendar className="w-4 h-4" /> },
    { id: 'tagging-warning', name: '태깅 미완료 경고', icon: <AlertCircle className="w-4 h-4" /> },
    { id: 'encouragement', name: '수업 완료 응원', icon: <MessageSquare className="w-4 h-4" /> },
    { id: 'next-booking', name: '다음 예약 유도', icon: <Calendar className="w-4 h-4" /> },
    { id: 'no-booking', name: '7일 무예약 알림', icon: <AlertCircle className="w-4 h-4" /> },
    { id: 'mid-month', name: '중간월 예약 부족', icon: <BarChart3 className="w-4 h-4" /> },
    { id: 'next-month', name: '다음달 예약 유도', icon: <Calendar className="w-4 h-4" /> }
  ];
    ...
    const mockReminders: Reminder[] = [
      {
        ...
        title: '수업 전일 리마인드',
        message: '내일 오후 2시에 한국어 수업이 있습니다. 준비물을 확인해주세요!',
        target: 'student',
        triggerTime: '18:00',
        isActive: true,
      ...
      },
      {
        ...
        title: '월초 예약 요약',
        message: '이번 달 예약 현황을 확인해보세요. 새로운 수업을 예약하시겠습니까?',
        target: 'student',
        triggerTime: '09:00',
        isActive: true,
      ...
      <div className="bg-white shadow-sm border-b">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center py-6">
            <div>
              <h1 className="text-3xl font-bold text-gray-900">자동 리마인드 시스템</h1>
                ...
                <div className="bg-white rounded-lg shadow-sm border p-4">
                  ...
                  <div className="space-y-3">
                    <div className="flex items-center justify-between">
                      <span className="text-sm text-gray-600">활성 리마인드</span>
                      <span className="font-medium">{reminders.filter(r => r.isActive).length}개</span>
                    </div>
                    <div className="flex items-center justify-between">
                      <span className="text-sm text-gray-600">총 발송</span>
                      <span className="font-medium">{reminders.reduce((sum, r) => sum + r.sentCount, 0)}회</span>
                    </div>
                  </div>
                </div>
              )}
          ...
          <div className="lg:col-span-3">
              ...
              <div className="space-y-6">
                ...
                <div className="bg-white rounded-lg shadow-sm border">
                  <div className="p-6 border-b border-gray-200">
                    <h2 className="text-lg font-semibold text-gray-900">리마인드 목록</h2>
                  </div>
                          ...
                          <div className="flex items-start justify-between">
                            <div className="flex-1">
                              <div className="flex items-center space-x-3 mb-2">
                                ...
                                <span className="text-sm text-gray-500">
                                  대상: {reminder.target === 'both' ? '학생/관리자' : reminder.target === 'student' ? '학생' : '관리자'}
                                </span>
                              </div>
                              ...
                              <div className="flex items-center space-x-4 text-sm text-gray-500">
                                ...
                                {reminder.lastSent && (
                                  <span>마지막 발송: {new Date(reminder.lastSent).toLocaleDateString()}</span>
                                )}
                              </div>
                            </div>
                            <div className="flex items-center space-x-2">
                              <button
                                ...
                                className={`p-2 rounded ${
                                  reminder.isActive ? 'text-green-600 hover:bg-green-50' : 'text-gray-400 hover:bg-gray-50'
                                }`}
                              >
                                {reminder.isActive ? <CheckCircle className="w-4 h-4" /> : <X className="w-4 h-4" />}
                              </button>
                              <button
            ...
            {activeTab === 'logs' && (
              <div className="space-y-6">
                ...
                <div className="bg-white rounded-lg shadow-sm border">
                  <div className="p-6 border-b border-gray-200">
                    <h2 className="text-lg font-semibold text-gray-900">발송 로그</h2>
                  </div>
                  <div className="divide-y divide-gray-200">
                          ...
                          <div className="flex items-start justify-between">
                            <div className="flex-1">
                              <div className="flex items-center space-x-3 mb-2">
                                ...
                                <span className="font-medium text-gray-900">{log.recipientName}</span>
                                <span className="text-sm text-gray-500">
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-sm text-gray-600">이메일을 통한 알림 발송</p>
                      </div>
                    ...
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-sm text-gray-600">LINE을 통한 알림 발송</p>
                      </div>
                      <button className="bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700">
                        활성화
                      </button>
                    ...
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-sm text-gray-600">웹 푸시 알림 발송</p>
                      </div>
                      <button className="bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700">
                        활성화
                      </button>
