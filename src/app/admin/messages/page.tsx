...
const AdminMessagesPage = () => {
    ...
    const mockMessages: Message[] = [
      {
        id: '1',
        title: '수업 일정 변경 안내',
        content: '안녕하세요. 다음 주 수업 일정이 변경되었습니다. 자세한 내용은 첨부된 파일을 확인해 주세요.',
        type: 'announcement',
    ...
    const mockTemplates: MessageTemplate[] = [
      {
        ...
        type: 'announcement',
        title: '수업 일정 변경 안내',
        content: '안녕하세요. {date} 수업 일정이 변경되었습니다. 자세한 내용은 첨부된 파일을 확인해 주세요.',
        category: '일정 관리',
        isDefault: true
      },
      {
      ...
      },
      {
        ...
        type: 'notification',
        title: '신규 코스 오픈 안내',
        content: '새로운 코스 "{course_name}"이 오픈되었습니다. 많은 관심 부탁드립니다.',
        category: '코스 관리',
        isDefault: false
      },
      {
      ...
      },
      {
        ...
        type: 'announcement',
        title: '시스템 점검 안내',
        content: '{date} {time}부터 {duration}시간 동안 시스템 점검이 있을 예정입니다.',
        category: '시스템 관리',
        isDefault: true
      }
    ];
  ...
  const handleSendMessage = (messageId: string) => {
    setMessages(messages.map(m => 
      m.id === messageId ? { ...m, status: 'sent', sentAt: new Date().toISOString().replace('T', ' ').substring(0, 16) } : m
    ));
  };
    ...
    <div className="p-6 bg-gray-50 min-h-screen">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-600">알림, 공지사항, 리마인더 메시지를 관리할 수 있습니다.</p>
        </div>
          ...
          <div className="bg-white rounded-lg shadow p-6">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                ...
                <p className="text-2xl font-bold text-blue-600">{messageStats.scheduled}</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-lg shadow p-6">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                ...
                <p className="text-2xl font-bold text-yellow-600">{messageStats.draft}</p>
              </div>
            </div>
          </div>
              ...
              <div className="p-6">
                ...
                <div className="flex items-center justify-between pt-4 border-t border-gray-200">
                  <div className="flex items-center gap-2 text-sm text-gray-500">
                    <span>읽음: {message.readCount}/{message.totalRecipients}</span>
                  </div>
                  <div className="flex gap-2">
                    <button
                      onClick={() => setSelectedMessage(message)}
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-xl font-bold text-gray-900">새 메시지 작성</h2>
              <button
            ...
            <div className="space-y-6">
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setNewMessageData({...newMessageData, title: e.target.value})}
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => setNewMessageData({...newMessageData, type: e.target.value as Message['type']})}
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                  >
                    <option value="notification">알림</option>
                    <option value="announcement">공지사항</option>
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setNewMessageData({...newMessageData, scheduledAt: e.target.value})}
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                  />
                </div>
              </div>
                ...
                <div className="space-y-2">
                  {recipientOptions.map((option) => (
                    <label key={option.value} className="flex items-center">
                      <input
                        type="checkbox"
                        checked={newMessageData.recipients.includes(option.value)}
                        onChange={(e) => {
                          if (e.target.checked) {
                            setNewMessageData({
                              ...newMessageData,
                              recipients: [...newMessageData.recipients, option.value]
                            });
                          } else {
                            setNewMessageData({
                              ...newMessageData,
                              recipients: newMessageData.recipients.filter(r => r !== option.value)
                            });
                          }
                        }}
              ...
              <div className="flex justify-end gap-3 pt-4">
                ...
                <button
                  ...
                  disabled={!newMessageData.title || !newMessageData.content || newMessageData.recipients.length === 0}
          ...
          <div className="bg-white rounded-lg p-6 w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-2 gap-6">
                <div>
                  ...
                  <div className="space-y-3">
                    <div>
                      ...
                      <span className="ml-2 text-sm text-gray-900">{selectedMessage.title}</span>
                    </div>
                    <div>
                      <span className="text-sm font-medium text-gray-500">유형:</span>
                      <span className={`ml-2 inline-flex px-2 py-1 text-xs font-semibold rounded-full ${getTypeColor(selectedMessage.type)}`}>
                ...
                <div>
                  <h3 className="text-lg font-semibold text-gray-900 mb-4">발송 정보</h3>
                  <div className="space-y-3">
                    <div>
                      <span className="text-sm font-medium text-gray-500">발신자:</span>
                      <span className="ml-2 text-sm text-gray-900">{selectedMessage.sender}</span>
                    </div>
                    <div>
                      <span className="text-sm font-medium text-gray-500">수신자:</span>
                      <span className="ml-2 text-sm text-gray-900">{getRecipientsText(selectedMessage.recipients)}</span>
                    </div>
                    <div>
                      <span className="text-sm font-medium text-gray-500">읽음:</span>
                      <span className="ml-2 text-sm text-gray-900">{selectedMessage.readCount}/{selectedMessage.totalRecipients}</span>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-6">
                <div>
                  <h3 className="text-lg font-semibold text-gray-900 mb-4">날짜 정보</h3>
                  <div className="space-y-2">
                    <div>
                      <span className="text-sm font-medium text-gray-500">생성일:</span>
                      <span className="ml-2 text-sm text-gray-900">{selectedMessage.createdAt}</span>
                    </div>
                    {selectedMessage.sentAt && (
                      <div>
                        <span className="text-sm font-medium text-gray-500">발송일:</span>
                        <span className="ml-2 text-sm text-gray-900">{selectedMessage.sentAt}</span>
                      </div>
                    )}
                    {selectedMessage.scheduledAt && (
                      <div>
                        <span className="text-sm font-medium text-gray-500">예약일:</span>
                        <span className="ml-2 text-sm text-gray-900">{selectedMessage.scheduledAt}</span>
                      </div>
                    )}
                  </div>
                </div>
