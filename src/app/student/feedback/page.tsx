...
export default function StudentFeedbackPage() {
    ...
    const mockFeedbacks: Feedback[] = [
      {
        ...
        teacherResponse: '열심히 공부하시는 모습이 보기 좋았습니다. 다음 수업에서도 좋은 성과 기대합니다!',
  ...
  const handleCategoryRatingChange = (category: keyof Feedback['categories'], rating: number) => {
    setCurrentFeedback(prev => ({
      ...prev,
      categories: {
        ...prev.categories!,
    ...
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-600">완료된 수업에 대한 피드백을 작성해주세요</p>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="bg-blue-50 p-4 rounded-lg">
              <div className="flex items-center justify-between">
                ...
                <span className="text-2xl font-bold text-blue-700">{lessons.length}</span>
              </div>
            </div>
            <div className="bg-green-50 p-4 rounded-lg">
              <div className="flex items-center justify-between">
                <span className="text-sm text-green-600">피드백 작성 완료</span>
                <span className="text-2xl font-bold text-green-700">{feedbacks.length}</span>
              </div>
            </div>
            <div className="bg-yellow-50 p-4 rounded-lg">
              <div className="flex items-center justify-between">
                <span className="text-sm text-yellow-600">미작성 수업</span>
                <span className="text-2xl font-bold text-yellow-700">{lessonsWithoutFeedback.length}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        {showFeedbackForm && selectedLesson && (
          <div className="bg-white rounded-lg shadow-md p-6 mb-6">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-lg font-semibold text-gray-900">피드백 작성</h2>
              <button
                ...
                <div className="space-y-6">
                  ...
                  <div className="bg-gray-50 p-4 rounded-lg">
                    ...
                    <div className="text-sm text-gray-600 space-y-1">
                      ...
                      <p>유형: {lesson.type === 'online' ? '온라인' : '대면'}</p>
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    {renderStars(currentFeedback.rating || 0, handleRatingChange)}
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                      ...
                      {renderStars(
                        ...
                        (rating) => handleCategoryRatingChange('teaching', rating)
                      )}
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-2">
                      ...
                      {renderStars(
                        ...
                        (rating) => handleCategoryRatingChange('materials', rating)
                      )}
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-2">
                      ...
                      {renderStars(
                        ...
                        (rating) => handleCategoryRatingChange('communication', rating)
                      )}
                    </div>
                  </div>
                  ...
                  <div className="flex items-center space-x-2">
                    <input
                      ...
                      onChange={(e) => setCurrentFeedback(prev => ({ ...prev, isPublic: e.target.checked }))}
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-sm text-gray-600">
                        {lesson.teacherName} • {lesson.date.toLocaleDateString()} • {lesson.duration}분
                      </p>
                    </div>
                    <button
                      onClick={() => {
                  ...
                  <div key={feedback.id} className="border border-gray-200 rounded-lg p-4">
                    <div className="flex items-start justify-between mb-3">
                      <div>
                        <h3 className="font-medium text-gray-900">{lesson.title}</h3>
                        <p className="text-sm text-gray-600">
                          {lesson.teacherName} • {lesson.date.toLocaleDateString()}
                        </p>
                      </div>
                      <div className="flex items-center space-x-2">
                    ...
                    <div className="grid grid-cols-3 gap-4 mb-3 text-sm">
                      <div>
                        ...
                        <div className="flex">{renderStars(feedback.categories.teaching)}</div>
                      </div>
                      <div>
                        <span className="text-gray-500">교재/자료:</span>
                        <div className="flex">{renderStars(feedback.categories.materials)}</div>
                      </div>
                      <div>
                        <span className="text-gray-500">소통:</span>
                        <div className="flex">{renderStars(feedback.categories.communication)}</div>
                      </div>
                    </div>
                    ...
                    {feedback.teacherResponse && (
                      <div className="bg-blue-50 p-3 rounded-lg">
                        ...
                        <p className="text-sm text-blue-800">{feedback.teacherResponse}</p>
                    ...
                    <div className="flex items-center justify-between mt-3 text-xs text-gray-500">
                      <span>작성일: {feedback.createdAt.toLocaleDateString()}</span>
                      {feedback.isPublic && (
