...
import { Home, User, CheckCircle, AlertCircle, BookOpen, Clock, MoreHorizontal, MessageSquare, ShoppingCart, Calendar } from 'lucide-react';
...
interface StudentTaggingResult {
  ...
  eventType: 'attendance' | 'manual_attendance' | 'no_reservation' | 'already_registered' | 'other';
  message: string;
  pointsEarned?: number;
...
export default function StudentTaggingPage() {
  ...
  const [currentResult, setCurrentResult] = useState<StudentTaggingResult | null>(null);
  const [showManualAttendance, setShowManualAttendance] = useState(false);
  const [showOtherOptions, setShowOtherOptions] = useState(false);
  const [selectedService, setSelectedService] = useState('');
  const [selectedTime, setSelectedTime] = useState('');
  const [selectedOtherType, setSelectedOtherType] = useState<'consultation' | 'materials' | 'other'>('consultation');
  ...
  const handleStudentTagging = async () => {
    ...
    try {
      ...
      setCurrentResult({
        ...
        message: error instanceof Error ? error.message : '카드 읽기에 실패했습니다'
      });
      setIsTagging(false);
    }
  };
              ...
              {currentResult.student && (
                <div className="text-center mb-4">
                  <div className="flex items-center justify-center gap-2 mb-2">
                    ...
                    <span className="text-lg font-medium">{currentResult.student.name}</span>
                  </div>
                  <div className="text-sm text-gray-600">
                    레벨 {currentResult.student.level} | 포인트 {currentResult.student.points}점
                  </div>
                ...
                <div className="text-center mb-4">
                  <div className="space-y-2 text-sm">
                    ...
                    <div className="flex items-center justify-center gap-2">
                      ...
                      <span>선생님: {currentResult.reservation.teacherName}</span>
                    </div>
                    <div className="flex items-center justify-center gap-2">
