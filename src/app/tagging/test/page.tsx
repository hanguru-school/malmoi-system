...
export default function TaggingSystemTestPage() {
  ...
  const testScenarios = [
    {
      ...
      run: async (): Promise<TestResult> => {
        ...
        const uid = 'TEST' + Math.random().toString(36).substr(2, 8).toUpperCase();
    ...
    },
    {
      ...
      run: async (): Promise<TestResult> => {
        ...
        try {
          ...
          // 등록되지 않은 UID로 테스트
          const unregisteredUid = 'UNKNOWN' + Math.random().toString(36).substr(2, 8).toUpperCase();
          const unknownUser = await taggingSystem.findUserByUID(unregisteredUid);
      ...
      run: async (): Promise<TestResult> => {
        ...
        try {
          ...
          const promises = Array.from({ length: concurrentTests }, (_, i) => {
            ...
            return taggingSystem.processTagging(
              uid,
              'perf-device',
              'nfc',
    ...
    },
    {
      ...
      run: async (): Promise<TestResult> => {
        ...
        try {
          const uid = 'CACHE' + Math.random().toString(36).substr(2, 8).toUpperCase();
  ...
  const runTests = useCallback(async (testIds?: string[]) => {
    setIsRunning(true);
    const testsToRun = testIds ? testScenarios.filter(t => testIds.includes(t.id)) : testScenarios;
    ...
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            <h1 className="text-3xl font-bold text-gray-900">태깅 시스템 테스트</h1>
            <p className="text-lg text-gray-600">태깅 시스템의 모든 기능을 종합적으로 테스트합니다</p>
          </div>
        ...
        <div className="bg-white rounded-xl shadow-lg p-6 mb-8">
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-xl font-semibold text-gray-900">테스트 컨트롤</h2>
            <div className="flex items-center gap-4">
              <button
                onClick={() => runTests(selectedTests.length > 0 ? selectedTests : undefined)}
                disabled={isRunning}
        ...
        {/* 테스트 시나리오 선택 */}
        <div className="bg-white rounded-xl shadow-lg p-6 mb-8">
          <h3 className="text-lg font-semibold text-gray-900 mb-4">테스트 시나리오 선택</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {testScenarios.map((scenario) => (
              <label key={scenario.id} className="flex items-start gap-3 p-4 border border-gray-200 rounded-lg hover:bg-gray-50 cursor-pointer">
                <input
                  type="checkbox"
                  ...
                  onChange={(e) => {
                    if (e.target.checked) {
                      ...
                      setSelectedTests(prev => prev.filter(id => id !== scenario.id));
                    }
                  }}
        ...
        <div className="bg-white rounded-xl shadow-lg p-6">
          ...
          {testResults.length === 0 ? (
            <div className="text-center py-12">
              ...
              <p className="text-gray-600">아직 테스트가 실행되지 않았습니다.</p>
            </div>
          ) : (
