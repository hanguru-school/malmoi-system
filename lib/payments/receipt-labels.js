...
export function classroomContactFooterText() {
  const email = String(process.env.CLASSROOM_CONTACT_EMAIL || "office@hanguru.blog").trim();
  return email ? `お問い合わせ: ${email}` : "";
}

export function officeInboxEmail() {
  return String(process.env.PAYMENT_OFFICE_EMAIL || "office@hanguru.blog").trim();
}
