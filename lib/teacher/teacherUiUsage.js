  ...
  try {
    ...
    if (Number(p.v) >= SCHEMA_V) {
      return {
        ...
        noteQuickTemplates:
          typeof p.noteQuickTemplates === "object" && p.noteQuickTemplates ? p.noteQuickTemplates : {},
        pointSnippets: typeof p.pointSnippets === "object" && p.pointSnippets ? p.pointSnippets : {},
        ...
        noteQuickTemplateRecent: Array.isArray(p.noteQuickTemplateRecent)
          ? p.noteQuickTemplateRecent.map(String)
          : [],
        pointSnippetRecent: Array.isArray(p.pointSnippetRecent) ? p.pointSnippetRecent.map(String) : [],
...
/** レッスンノート画面のクイック定型（structure / review / mini 等） */
export function recordTeacherNoteQuickTemplateUse(templateId) {
  const id = String(templateId || "").trim();
  if (!id) return;
  const s = readState();
  writeState({
    ...s,
    noteQuickTemplates: bumpEntry(s.noteQuickTemplates || {}, id),
    noteQuickTemplateRecent: pushRecent(s.noteQuickTemplateRecent || [], id),
  });
}

/** 学習ポイントワンタップ（TEACHER_POINT_SNIPPETS の id） */
export function recordTeacherPointSnippetUse(snippetId) {
  const id = String(snippetId || "").trim();
  if (!id) return;
  const s = readState();
  writeState({
    ...s,
    pointSnippets: bumpEntry(s.pointSnippets || {}, id),
    pointSnippetRecent: pushRecent(s.pointSnippetRecent || [], id),
  });
}
...
function sortIdsByUsageMap(baseIds, usageMap, recentList) {
  const base = [...baseIds];
  const recent = recentList || [];
  const rank = new Map(recent.map((id, i) => [id, i]));
  return [...base].sort((a, b) => {
    const ea = entryScore(usageMap[a]);
    const eb = entryScore(usageMap[b]);
...
export function sortPresetIdsByUsage(defaultOrder = []) {
  ...
  return sortIdsByUsageMap(base, s.presets || {}, s.presetRecent || []);
}
 ...
 * @param {Array<{ id: string }>} defaultOrder
 */
export function sortNoteQuickTemplateIdsByUsage(defaultOrder = []) {
  const s = readState();
  const base = defaultOrder.map((x) => String(x.id || "").trim()).filter(Boolean);
  return sortIdsByUsageMap(base, s.noteQuickTemplates || {}, s.noteQuickTemplateRecent || []);
}
 ...
 * @param {Array<{ id: string }>} defaultOrder
 */
export function sortPointSnippetIdsByUsage(defaultOrder = []) {
  const s = readState();
  const base = defaultOrder.map((x) => String(x.id || "").trim()).filter(Boolean);
  return sortIdsByUsageMap(base, s.pointSnippets || {}, s.pointSnippetRecent || []);
}

/**
 * 宿題「保存テンプレ」一覧の id 並び（API から取得した { id } 配列向け）
 * @param {Array<{ id: string }>} templatesFromApi
 */
export function sortHomeworkTemplateRowsByUsage(templatesFromApi = []) {
  const s = readState();
  const base = templatesFromApi.map((x) => String(x.id || "").trim()).filter(Boolean);
  const orderedIds = sortIdsByUsageMap(base, s.templates || {}, s.templateRecent || []);
  const map = new Map(templatesFromApi.map((row) => [String(row.id || "").trim(), row]));
