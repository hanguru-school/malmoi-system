...
export default function TeacherLessonEditorPage() {
  ...
  const getClassesByDate = (date: string): ClassInfo[] => {
    const allClasses: { [key: string]: ClassInfo[] } = {
      '2024-01-16': [
        { studentName: '김학생', time: '09:00', subject: '문법', date: '2024-01-16' },
        { studentName: '이학생', time: '11:00', subject: '회화', date: '2024-01-16' },
        { studentName: '박학생', time: '14:00', subject: '독해', date: '2024-01-16' },
        { studentName: '최학생', time: '16:00', subject: '듣기', date: '2024-01-16' }
      ],
      '2024-01-17': [
        { studentName: '정학생', time: '10:00', subject: '문법', date: '2024-01-17' },
        { studentName: '한학생', time: '15:00', subject: '회화', date: '2024-01-17' }
      ],
      '2024-01-18': [] // 수업이 없는 날
    };
  ...
  useEffect(() => {
    ...
    if (studentName && time && subject) {
      setSelectedClass({ studentName, time, subject, date: date || new Date().toISOString().split('T')[0] });
    }
    ...
    setTimeout(() => {
      setLessonNote({
        ...
        content: '<h2>오늘의 수업 내용</h2><p>안녕하세요! 오늘은 한국어 기초 문법을 배워보겠습니다.</p><h3>1. 인사말</h3><ul><li>안녕하세요 - Hello</li><li>안녕히 가세요 - Goodbye</li><li>감사합니다 - Thank you</li></ul><h3>2. 자기소개</h3><p>저는 [이름]입니다. 만나서 반갑습니다.</p>',
    ...
    <div className="p-6 space-y-6">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-gray-600">
              {selectedClass ? `${selectedClass.studentName} - ${selectedClass.time} (${selectedClass.subject})` : '수업 선택 필요'}
            </p>
          </div>
        </div>
              ...
              <div className="space-y-3">
                {availableClasses.map((classItem) => (
                  ...
                  >
                    ...
                    <div className="text-sm text-gray-600">{classItem.time} - {classItem.subject}</div>
                  </button>
                ))}
              </div>
            ) : (
      ...
      {!selectedClass && availableClasses.length === 0 && (
        <div className="text-center py-12">
          ...
          <h3 className="text-lg font-medium text-gray-900 mb-2">해당 날짜에 수업이 없습니다</h3>
          <p className="text-gray-600 mb-4">선택한 날짜에 예정된 수업이 없어 노트를 작성할 수 없습니다.</p>
          <Link
            href="/teacher/home"
      ...
      {!selectedClass && availableClasses.length > 0 && (
        <div className="text-center py-12">
          ...
          <p className="text-gray-600 mb-4">해당 날짜에 {availableClasses.length}개의 수업이 있습니다.</p>
          <button
            onClick={() => setShowClassSelector(true)}
            className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700"
          >
      ...
      {selectedClass && lessonNote && (
        <>
          ...
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
            <h3 className="font-semibold text-blue-900 mb-2">오늘 해야 할 수업 내용</h3>
            ...
            {lessonNote.homework.hasHomework && (
              <div className="mt-2">
                ...
                <p className="text-blue-800">{lessonNote.homework.studentHomework}</p>
              </div>
            )}
          </div>
          ...
          <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
            <h3 className="font-semibold text-yellow-900 mb-2">이전 수업 메모</h3>
            <div className="space-y-2">
              <div>
                <span className="text-sm font-medium text-yellow-800">다음 수업 내용:</span>
                <p className="text-yellow-800">{lessonNote.nextLesson || '메모가 없습니다.'}</p>
              </div>
              {lessonNote.homework.hasHomework && (
                <div>
                  <span className="text-sm font-medium text-yellow-800">이전 숙제:</span>
                  <p className="text-yellow-800">{lessonNote.homework.studentHomework}</p>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="bg-white rounded-lg shadow-sm border p-4">
            ...
            {lessonNote.homework.hasHomework && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">선생님 숙제</label>
                  <textarea
                    value={lessonNote.homework.teacherHomework}
                    onChange={(e) => updateHomework('teacherHomework', e.target.value)}
                    className="w-full h-20 p-2 border border-gray-300 rounded resize-none"
                    placeholder="선생님이 준비해야 할 숙제 내용"
                  />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">학생 숙제</label>
                  <textarea
                    value={lessonNote.homework.studentHomework}
                    onChange={(e) => updateHomework('studentHomework', e.target.value)}
                    className="w-full h-20 p-2 border border-gray-300 rounded resize-none"
                  ...
                  <div className="flex gap-1">
                    {[1, 2, 3, 4, 5].map((star) => (
                      <button
                        ...
                        className={`p-1 ${
                          (lessonNote.evaluation[item.key as keyof typeof lessonNote.evaluation] as number) >= star
                            ? 'text-yellow-500'
                            : 'text-gray-300'
                        }`}
                      >
      ...
      {showCommentModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 w-full max-w-md">
            ...
            <textarea
              id="commentInput"
              defaultValue={lessonNote?.evaluation[currentEvaluationField + 'Comment' as keyof typeof lessonNote.evaluation] as string}
        ...
        <div className="fixed inset-0 bg-white z-50 p-8">
          <div className="max-w-4xl mx-auto">
            <div className="text-center mb-8">
              ...
              <p className="text-gray-600">{new Date().toLocaleDateString('ko-KR')}</p>
            </div>
            <div 
