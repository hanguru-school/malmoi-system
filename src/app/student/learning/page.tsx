...
export default function StudentLearningPage() {
  const [activeTab, setActiveTab] = useState<'practice' | 'writing' | 'exam' | 'recordings'>('practice');
    ...
    const mockContents: LearningContent[] = [
      {
        ...
        type: 'listening',
        title: '기본 인사말 듣기',
        description: '안녕하세요, 만나서 반갑습니다 등의 기본 인사말을 듣고 따라해보세요.',
        level: '초급 A',
        duration: 120,
      ...
      },
      {
        ...
        type: 'speaking',
        title: '자기소개 연습',
        description: '저는 [이름]입니다. 한국어를 배우고 있어요. 등의 자기소개를 연습해보세요.',
        level: '초급 A',
        duration: 180,
    ...
    const mockWritingTests: WritingTest[] = [
      {
        id: '1',
        title: '자기소개 작문',
        content: '안녕하세요. 저는 김학생입니다. 한국어를 배우고 있어요. 만나서 반갑습니다.',
        score: 85,
            ...
            <div className="p-6">
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-2">
                  {content.type === 'listening' && <Headphones className="w-5 h-5 text-blue-600" />}
                  {content.type === 'speaking' && <Mic className="w-5 h-5 text-green-600" />}
                  {content.type === 'writing' && <FileText className="w-5 h-5 text-purple-600" />}
                  {content.type === 'exam' && <Award className="w-5 h-5 text-orange-600" />}
                  <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                    content.type === 'listening' ? 'bg-blue-100 text-blue-800' :
                    content.type === 'speaking' ? 'bg-green-100 text-green-800' :
                    content.type === 'writing' ? 'bg-purple-100 text-purple-800' :
                    'bg-orange-100 text-orange-800'
                  }`}>
                    {content.type === 'listening' ? '듣기' :
                     content.type === 'speaking' ? '말하기' :
                     content.type === 'writing' ? '작문' : '시험'}
                  </span>
                </div>
                {content.completed && (
              ...
              {content.audioUrl && (
                <div className="space-y-3">
                  <div className="bg-gray-50 rounded-lg p-3">
                    <p className="text-sm font-medium text-gray-900 mb-2">듣기 텍스트:</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {writingTests.map((test) => (
          <div key={test.id} className="bg-white rounded-xl shadow-lg p-6">
            ...
            <div className="space-y-3">
              <div>
                <p className="text-sm font-medium text-gray-700 mb-1">작문 내용:</p>
                <p className="text-sm text-gray-600 bg-gray-50 rounded p-3">{test.content}</p>
              </div>
              
              {test.feedback && (
                <div>
                  <p className="text-sm font-medium text-gray-700 mb-1">피드백:</p>
                  <p className="text-sm text-gray-600 bg-green-50 rounded p-3">{test.feedback}</p>
                </div>
              )}
      ...
      <div className="bg-white rounded-xl shadow-lg p-6">
        ...
        <div className="mt-6">
          ...
          <div className="space-y-2">
            {['기본 인사말', '자기소개', '숫자와 시간', '음식 주문', '교통수단'].map((item, index) => (
    ...
    <div className="min-h-screen bg-gray-50 p-4">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-8">
          ...
          <p className="text-lg text-gray-600">듣기, 말하기, 작문 연습을 통해 한국어 실력을 향상시키세요</p>
        </div>
          ...
          <div className="flex border-b">
            {[
              ...
              return (
                <button
                  key={tab.id}
                  onClick={() => setActiveTab(tab.id as 'practice' | 'writing' | 'exam' | 'recordings')}
