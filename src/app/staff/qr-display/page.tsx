...
export default function StaffQRDisplayPage() {
  ...
  useEffect(() => {
    ...
    // QR코드 데이터 생성 (직원 prefix: ST-)
    const qrData = `ST-${mockStaffInfo.id}-${Date.now()}`;
    setQrCodeData(qrData);
  }, []);

  const generateNewQR = () => {
    if (staffInfo) {
      const newQrData = `ST-${staffInfo.id}-${Date.now()}`;
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-gray-600">직원 전용 QR코드를 표시합니다</p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-green-50 border border-green-200 rounded-lg p-4">
        ...
        <ul className="text-sm text-green-800 space-y-1">
          <li>• 이 QR코드는 직원 전용으로 생성되었습니다 (접두사: ST-)</li>
