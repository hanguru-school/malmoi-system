...
/** 月表示用セル (月曜始まり、最大42セル) */
export function buildMonthGridCells(anchorIso) {
  const base = parseYmd(anchorIso) || new Date();
  const y = base.getFullYear();
  const m = base.getMonth();
...
/** fromIso ~ toIso までの日付文字列 (포함) */
export function listDaysInclusive(fromIso, toIso) {
  const out = [];
  let cur = String(fromIso || "").slice(0, 10);
