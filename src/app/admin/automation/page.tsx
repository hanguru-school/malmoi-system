...
import { automationSystem, messageTemplateManager, AutomationRule, MessageTemplate, AutomationLog } from '@/lib/automation-system';
...
export default function AutomationPage() {
  ...
  const handleRuleToggle = (ruleId: string) => {
    ...
    // 실제 시스템에 반영
    automationSystem.updateRule(ruleId, { enabled: !rules.find(r => r.id === ruleId)?.enabled });
  };
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8">
          <div>
            ...
            <p className="text-gray-600 mt-2">자동화 규칙 및 메시지 템플릿 관리</p>
          </div>
        ...
        <div className="space-y-8">
          {activeTab === 'rules' && (
            <div className="bg-white rounded-lg shadow-md">
              <div className="p-6">
                <h2 className="text-xl font-semibold text-gray-800 mb-4">자동화 규칙 목록</h2>
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex-1">
                          <div className="flex items-center space-x-3">
                            ...
                            <span className={`px-2 py-1 text-xs rounded-full ${
                              rule.type === 'reminder' ? 'bg-blue-100 text-blue-800' :
                              rule.type === 'notification' ? 'bg-yellow-100 text-yellow-800' :
                              'bg-purple-100 text-purple-800'
                            }`}>
                              {rule.type === 'reminder' ? '리마인드' :
                               rule.type === 'notification' ? '알림' : '리포트'}
                            </span>
                          </div>
                          <p className="text-sm text-gray-600 mt-1">
                            대상: {rule.conditions.targetType === 'student' ? '학생' :
                                   rule.conditions.targetType === 'teacher' ? '선생님' : '관리자'}
                          </p>
                          <p className="text-sm text-gray-600">
                            스케줄: {rule.schedule.type === 'daily' ? '매일' :
                                   rule.schedule.type === 'weekly' ? '매주' :
                                   rule.schedule.type === 'monthly' ? '매월' : '커스텀'}
          ...
          {activeTab === 'templates' && (
            <div className="bg-white rounded-lg shadow-md">
              <div className="p-6">
                <h2 className="text-xl font-semibold text-gray-800 mb-4">메시지 템플릿 목록</h2>
                <div className="space-y-4">
                        ...
                        <div className="flex-1">
                          <div className="flex items-center space-x-3">
                            ...
                            <span className={`px-2 py-1 text-xs rounded-full ${
                              ...
                              template.type === 'notification' ? 'bg-yellow-100 text-yellow-800' :
                              'bg-purple-100 text-purple-800'
                            }`}>
                              {template.type === 'reminder' ? '리마인드' :
                               template.type === 'notification' ? '알림' : '리포트'}
                            </span>
                          </div>
                          <p className="text-sm text-gray-600 mt-1">
                            제목: {template.title}
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white rounded-lg p-6 w-full max-w-2xl max-h-[90vh] overflow-y-auto">
              ...
              <TemplateForm
                ...
                onSave={(template) => {
                  if (editingTemplate) {
                    messageTemplateManager.updateTemplate(template.id, template);
                  } else {
...
function RuleForm({ rule, onSave, onCancel }: {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-2">규칙 이름</label>
        <input
          type="text"
          value={formData.name}
      ...
      <div>
        ...
        <select
          value={formData.type}
          onChange={(e) => setFormData({ ...formData, type: e.target.value as 'reminder' | 'notification' | 'report' })}
          className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
        >
      ...
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-2">스케줄 유형</label>
        <select
          value={formData.schedule?.type}
          onChange={(e) => setFormData({
            ...formData,
            schedule: { ...formData.schedule, type: e.target.value as 'daily' | 'weekly' | 'monthly' | 'custom' }
          })}
          className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
        >
      ...
      <div>
        ...
        <select
          ...
          onChange={(e) => setFormData({
            ...formData,
            conditions: { ...formData.conditions, targetType: e.target.value as 'student' | 'teacher' | 'admin' }
          })}
          className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
        >
      ...
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-2">메시지 제목</label>
        <input
          type="text"
        ...
        <div className="space-y-2">
          {['line', 'email', 'sms'].map((channel) => (
            <label key={channel} className="flex items-center">
              <input
                type="checkbox"
                checked={formData.channels?.includes(channel as 'line' | 'email' | 'sms')}
                onChange={(e) => {
                ...
                className="mr-2"
              />
              {channel === 'line' ? 'LINE' : channel === 'email' ? '이메일' : 'SMS'}
            </label>
          ))}
        </div>
      </div>
