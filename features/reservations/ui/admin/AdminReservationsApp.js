...
/**
 * 管理画面 予約運用 V2（features/reservations/ui）
 */
...
export default function AdminReservationsApp({ initialReservations = [], initialFilters = {}, scopeNotice = "" }) {
  ...
  const [lessonDeliveryFilter, setLessonDeliveryFilter] = useState("");
  ...
  const filtered = useMemo(() => {
    return [...reservations]
      ...
      .filter((item) => {
        if (!lessonDeliveryFilter) return true;
        return String(item.lessonDeliveryType || "in_person") === lessonDeliveryFilter;
      })
      .filter((item) => {
  ...
  }, [reservations, statusFilter, lessonDeliveryFilter, query]);
  ...
  async function load() {
    ...
    try {
      const [rData, sData, stData] = await Promise.all([
        ...
        fetchAdminReservationSlotsRange(fromDate, fromDate),
  ...
  async function updateReservation(id, patch) {
    ...
    setError("");
    try {
      await patchAdminReservation(id, patch);
      await load();
      return true;
    } catch (e) {
  ...
  async function createReservation() {
    ...
    try {
      ...
      await postAdminReservationCreate(payload);
      setOpenCreate(false);
      setCreateForm({
        mode: "single",
        slotId: "",
  ...
  function renderReservationDetail() {
    if (!selected) {
      return <p className={adminStyles.smallMuted}>一覧から行を選択すると、詳細と操作が表示されます。</p>;
    }
    return (
      <>
      ...
      <div className={v2.toolbar}>
        ...
        <label className={styles.label}>
          ...
          <select
            ...
            value={lessonDeliveryFilter}
            onChange={(e) => setLessonDeliveryFilter(e.target.value)}
          >
            <option value="">すべて</option>
