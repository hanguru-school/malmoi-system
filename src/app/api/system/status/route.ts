...
export async function GET(request: NextRequest) {
  try {
    ...
    const systemStatus = {
      ...
      services: {
        database: {
          ...
          connection_string: process.env.DATABASE_URL ? 'configured' : 'missing'
        },
        cognito: {
    ...
    if (validation.missingVars.length > 0) {
      systemStatus.recommendations.push(`Missing ${validation.missingVars.length} environment variables`);
    }
    
    if (validation.invalidVars.length > 0) {
      systemStatus.recommendations.push(`Invalid ${validation.invalidVars.length} environment variables`);
    }
    
    if (validation.warnings.length > 0) {
      systemStatus.recommendations.push(`${validation.warnings.length} configuration warnings`);
    }
