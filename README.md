...
2. 서버 **`/srv/malmoi/apps/malmoi-integrated/current`** 에 clone 후 `chmod +x deploy/deploy-prod.sh`
3. 로컬에 `scripts/deploy.env` 생성 (`scripts/deploy.env.example` 참고)
4. 배포: `npm run deploy` / `npm run deploy:server` / `bash deploy-now.sh` 또는 Cursor 작업 **MalMoi: Push main + deploy (SSH)**
