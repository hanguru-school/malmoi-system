...
export function isProductionEnvironment(): boolean {
  ...
  return currentUrl.includes(PRODUCTION_URL) || currentUrl.includes('app.hanguru.school');
}
...
export function isLocalDevelopment(): boolean {
  ...
  const currentUrl = window.location.href;
  return currentUrl.includes('localhost') || currentUrl.includes('127.0.0.1') || currentUrl.includes(':3000') || currentUrl.includes(':3006');
}
...
export function isTestEnvironment(): boolean {
  ...
  const currentUrl = window.location.href;
  return currentUrl.includes('vercel.app') || currentUrl.includes('netlify.app') || currentUrl.includes('preview');
}

export function shouldShowWarning(): boolean {
  return !isProductionEnvironment() && (isLocalDevelopment() || isTestEnvironment());
}
