...
interface PointRule {
  ...
  name: string;
  description: string;
  category: 'study' | 'review' | 'payment' | 'attendance' | 'homework' | 'other';
  points: number;
  autoGrant: boolean;
...
const PointsPage = () => {
  ...
  const [editingPaymentRule, setEditingPaymentRule] = useState<PaymentPointRule | null>(null);
  ...
  const handleSaveRule = () => {
    if (newRule.name && newRule.points > 0) {
      ...
      if (editingRule) {
        setPointRules(prev => prev.map(r => r.id === editingRule.id ? rule : r));
      } else {
  ...
  const handleSavePaymentRule = () => {
    if (newPaymentRule.name && newPaymentRule.value > 0) {
      ...
      if (editingPaymentRule) {
        setPaymentRules(prev => prev.map(r => r.id === editingPaymentRule.id ? rule : r));
      } else {
  ...
  const togglePaymentRuleStatus = (id: string) => {
    setPaymentRules(prev => prev.map(rule => 
      rule.id === id ? { ...rule, isActive: !rule.isActive } : rule
    ));
  };
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-600">포인트 부여 규칙과 결제 적립 규칙을 관리하세요</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-lg shadow-sm border p-6">
          <div className="flex justify-between items-center mb-6">
            <h2 className="text-lg font-semibold text-gray-900">포인트 부여 규칙</h2>
            <button
              ...
              <div key={rule.id} className="border border-gray-200 rounded-lg p-4">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <div className="flex items-center gap-2">
                    <span className="text-lg font-bold text-blue-600">{rule.points}P</span>
                    <button
                ...
                <div className="flex items-center gap-4 text-sm text-gray-600 mb-3">
                  <span className={`px-2 py-1 rounded-full ${getCategoryColor(rule.category)}`}>
                    {getCategoryText(rule.category)}
                  </span>
                  <span className={`px-2 py-1 rounded-full ${
                    rule.autoGrant ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'
                  }`}>
        ...
        <div className="bg-white rounded-lg shadow-sm border p-6">
          <div className="flex justify-between items-center mb-6">
            <h2 className="text-lg font-semibold text-gray-900">결제 적립 규칙</h2>
            <button
            ...
            {paymentRules.map((rule) => (
              <div key={rule.id} className="border border-gray-200 rounded-lg p-4">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-600">
                        {rule.type === 'percentage' ? `${rule.value}% 적립` : `${rule.value}P 적립`}
                        {rule.minAmount && rule.minAmount > 0 && ` (최소 ${rule.minAmount.toLocaleString()}원)`}
                      </p>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                    <span className="text-lg font-bold text-green-600">
                      {rule.type === 'percentage' ? `${rule.value}%` : `${rule.value}P`}
                    </span>
                    <button
                ...
                <div className="flex items-center gap-2 text-sm text-gray-600">
                  <span className={`px-2 py-1 rounded-full ${
                    rule.type === 'percentage' ? 'bg-blue-100 text-blue-800' : 'bg-orange-100 text-orange-800'
                  }`}>
      ...
      {showAddRuleModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 w-full max-w-2xl">
            ...
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">규칙명</label>
                <input
                  type="text"
                  value={newRule.name}
                ...
                <div className="flex items-center gap-2">
                  <input
                    ...
                    onChange={(e) => setNewRule(prev => ({ 
                      ...
                      requiresApproval: e.target.checked ? false : prev.requiresApproval
                    }))}
                    className="rounded"
                  />
                  <label htmlFor="autoGrant" className="text-sm text-gray-700">자동 부여</label>
                </div>
                <div className="flex items-center gap-2">
                  <input
                    type="checkbox"
                    ...
                    className="rounded"
                  />
                  <label htmlFor="requiresApproval" className="text-sm text-gray-700">승인 필요</label>
                </div>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">조건 (선택사항)</label>
                ...
                <button
                  onClick={() => setNewRule(prev => ({ ...prev, conditions: [...prev.conditions, ''] }))}
      ...
      {showAddPaymentRuleModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 w-full max-w-md">
            <h3 className="text-lg font-semibold text-gray-900 mb-4">
              {editingPaymentRule ? '결제 적립 규칙 수정' : '결제 적립 규칙 추가'}
            </h3>
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">규칙명</label>
                <input
                  ...
                  onChange={(e) => setNewPaymentRule(prev => ({ ...prev, name: e.target.value }))}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">적립 유형</label>
                  <select
                    value={newPaymentRule.type}
                    onChange={(e) => setNewPaymentRule(prev => ({ ...prev, type: e.target.value as any }))}
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                  >
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">
                    {newPaymentRule.type === 'percentage' ? '적립률 (%)' : '적립 포인트'}
                  </label>
                  <input
                    type="number"
                    value={newPaymentRule.value}
                    onChange={(e) => setNewPaymentRule(prev => ({ ...prev, value: parseFloat(e.target.value) }))}
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                    min="0"
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setNewPaymentRule(prev => ({ ...prev, minAmount: parseInt(e.target.value) }))}
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                    min="0"
                  />
                </div>
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setNewPaymentRule(prev => ({ ...prev, maxAmount: parseInt(e.target.value) }))}
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                    min="0"
                    placeholder="0 = 제한없음"
                  />
