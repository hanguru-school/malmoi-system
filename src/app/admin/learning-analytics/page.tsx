...
import { Users, Calendar, CheckCircle, Star, AlertCircle, Search, Eye, MessageSquare } from 'lucide-react';
...
export default function AdminLearningAnalyticsPage() {
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'students' | 'levels' | 'trends'>('overview');
  const [searchTerm, setSearchTerm] = useState('');
  const [levelFilter, setLevelFilter] = useState<'all' | 'A-1' | 'A-2' | 'B-1' | 'B-2'>('all');
  const [sortBy, setSortBy] = useState<'name' | 'progress' | 'rating' | 'lessons'>('progress');
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      ...
      const mockMonthlyTrends: MonthlyTrend[] = [
        { month: '7월', newStudents: 12, activeStudents: 120, totalLessons: 156, averageRating: 4.2, completionRate: 75 },
        { month: '8월', newStudents: 15, activeStudents: 125, totalLessons: 168, averageRating: 4.3, completionRate: 78 },
        { month: '9월', newStudents: 18, activeStudents: 130, totalLessons: 175, averageRating: 4.1, completionRate: 72 },
        { month: '10월', newStudents: 22, activeStudents: 135, totalLessons: 182, averageRating: 4.4, completionRate: 80 },
        { month: '11월', newStudents: 20, activeStudents: 140, totalLessons: 188, averageRating: 4.3, completionRate: 78 },
        { month: '12월', newStudents: 25, activeStudents: 145, totalLessons: 195, averageRating: 4.5, completionRate: 82 }
      ];
    ...
    <div className="max-w-7xl mx-auto">
      ...
      <div className="bg-white rounded-xl shadow-lg mb-8">
        ...
        <div className="p-6">
            ...
            <div className="space-y-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                ...
                <div className="bg-green-50 rounded-lg p-6">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <div className="text-2xl font-bold text-green-900">{overallStats.totalLessons}</div>
                    </div>
                ...
                <div className="bg-purple-50 rounded-lg p-6">
                  ...
                  <div className="mt-2 text-sm text-purple-700">
                    {overallStats.completedHomework}/{overallStats.totalHomework}개 완료
                  </div>
                </div>
                ...
                <div className="bg-orange-50 rounded-lg p-6">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <div className="text-2xl font-bold text-orange-900">{overallStats.averageRating}</div>
                    </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-6">
                  <h3 className="font-semibold text-yellow-900 mb-4">주요 개선점</h3>
                ...
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-6">
                  <h3 className="font-semibold text-blue-900 mb-4">권장 조치사항</h3>
          ...
          {activeTab === 'students' && (
            <div className="space-y-6">
              ...
              <div className="flex flex-col md:flex-row gap-4">
                ...
                <select
                  value={levelFilter}
                  onChange={(e) => setLevelFilter(e.target.value as 'all' | 'A-1' | 'A-2' | 'B-1' | 'B-2')}
                ...
                <select
                  value={sortBy}
                  onChange={(e) => setSortBy(e.target.value as 'progress' | 'rating' | 'lessons' | 'name')}
                  className="px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                >
                ...
                <div className="overflow-x-auto">
                  <table className="w-full">
                    <thead className="bg-gray-50">
                      <tr>
                        <th className="text-left py-3 px-4 font-medium text-gray-900">학생명</th>
                        <th className="text-left py-3 px-4 font-medium text-gray-900">레벨</th>
                        <th className="text-center py-3 px-4 font-medium text-gray-900">진행도</th>
                        <th className="text-center py-3 px-4 font-medium text-gray-900">수업</th>
                        <th className="text-center py-3 px-4 font-medium text-gray-900">숙제 완료율</th>
                        <th className="text-center py-3 px-4 font-medium text-gray-900">평균 별점</th>
                    ...
                    <tbody>
                      {sortedStudents.map((student) => (
                        <tr key={student.studentId} className="border-b border-gray-100">
                          <td className="py-3 px-4">
                            <div>
                              ...
                              <div className="text-sm text-gray-600">
                                마지막 수업: {new Date(student.lastLessonDate).toLocaleDateString('ko-KR')}
                              </div>
                            </div>
                          </td>
                          <td className="py-3 px-4">
                          ...
                          <td className="py-3 px-4 text-center">
                            <span className={`font-medium ${getProgressColor(student.homeworkCompletion)}`}>
          ...
          {activeTab === 'levels' && (
            <div className="space-y-6">
              <h3 className="text-lg font-semibold text-gray-900">레벨별 분석</h3>
              
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {levelAnalysis.map((level) => (
                  <div key={level.level} className="bg-white border border-gray-200 rounded-lg p-6">
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="flex items-center gap-2">
                        <Star className="w-4 h-4 text-yellow-400 fill-current" />
                        <span className="text-sm text-gray-600">평균 별점: {level.averageRating}</span>
                      </div>
                      
                      <div>
                        <h5 className="font-medium text-gray-900 mb-2">주요 약점</h5>
                      ...
                      <div>
                        <h5 className="font-medium text-gray-900 mb-2">권장 조치</h5>
                        <ul className="text-sm text-gray-600 space-y-1">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {monthlyTrends.map((trend, index) => (
                  <div key={index} className="bg-white border border-gray-200 rounded-lg p-6">
                    <h4 className="font-semibold text-gray-900 mb-4">{trend.month}</h4>
                    
                    <div className="space-y-3">
                      <div className="flex justify-between items-center">
                        <span className="text-sm text-gray-600">신규 학생</span>
                        <span className="font-medium text-green-600">+{trend.newStudents}명</span>
                      </div>
                      
                      <div className="flex justify-between items-center">
                        <span className="text-sm text-gray-600">활성 학생</span>
                        <span className="font-medium">{trend.activeStudents}명</span>
                      </div>
                      
                      <div className="flex justify-between items-center">
                        <span className="text-sm text-gray-600">총 수업</span>
                        <span className="font-medium">{trend.totalLessons}회</span>
                      </div>
