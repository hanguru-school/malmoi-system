  ...
  try {
    ...
    if (reservationsResult.rows.length > 0) {
      ...
      reservationsResult.rows.forEach((reservation, index) => {
        console.log(`${index + 1}. ID: ${reservation.id}, 날짜: ${reservation.date}, 시간: ${reservation.start_time} ~ ${reservation.end_time}`);
      });
    ...
    for (const table of relatedTables) {
      try {
        const deleteRelatedResult = await pool.query(`DELETE FROM ${table} WHERE reservation_id IS NOT NULL`);
        console.log(`${table} 테이블에서 예약 관련 데이터 ${deleteRelatedResult.rowCount}개 삭제`);
      } catch (error) {
