...
export async function POST(request: NextRequest) {
  try {
    ...
    switch (body.action) {
      case 'check_in':
        const checkInResult = await processEmployeeCheckIn(body.uid, employeeInfo);
        responseData = { ...responseData, ...checkInResult };
        break;
        
      case 'check_out':
        const checkOutResult = await processEmployeeCheckOut(body.uid, employeeInfo);
        responseData = { ...responseData, ...checkOutResult };
        break;
        
      case 're_check_in':
        const reCheckInResult = await processEmployeeReCheckIn(body.uid, employeeInfo, body.transportationFee);
...
async function processEmployeeCheckIn(uid: string, employeeInfo: any): Promise<any> {
  const now = new Date();
...
async function processEmployeeCheckOut(uid: string, employeeInfo: any): Promise<any> {
  const now = new Date();
...
async function processEmployeeReCheckIn(uid: string, employeeInfo: any, transportationFee?: boolean): Promise<any> {
  const now = new Date();
...
async function recordEmployeeCheckIn(uid: string, checkInTime: Date): Promise<void> {
  // 출근 기록 저장
  console.log(`직원 출근 기록: ${uid} at ${checkInTime.toISOString()}`);
}

async function recordEmployeeCheckOut(uid: string, checkOutTime: Date, workHours: number): Promise<void> {
  // 퇴근 기록 저장
  console.log(`직원 퇴근 기록: ${uid} at ${checkOutTime.toISOString()}, 근무시간: ${workHours}시간`);
}

async function recordEmployeeReCheckIn(uid: string, checkInTime: Date, transportationFee?: boolean): Promise<void> {
  // 재출근 기록 저장
  console.log(`직원 재출근 기록: ${uid} at ${checkInTime.toISOString()}, 교통비: ${transportationFee ? '청구' : '미청구'}`);
}
...
export async function GET(request: NextRequest) {
  try {
    ...
    const todayCheckIn = employeeInfo.lastCheckIn && isSameDay(employeeInfo.lastCheckIn, today);
    const todayCheckOut = employeeInfo.lastCheckOut && isSameDay(employeeInfo.lastCheckOut, today);
