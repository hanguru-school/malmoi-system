...
export default function AutomationManagementPage() {
  ...
  const [activeTab, setActiveTab] = useState<'rules' | 'logs' | 'queue' | 'analytics'>('rules');
                    ...
                    <div className="flex items-start justify-between mb-4">
                      <div className="flex-1">
                        <div className="flex items-center gap-3 mb-2">
                          ...
                          <span className={`px-2 py-1 text-xs rounded-full ${
                            rule.type === 'schedule' ? 'bg-blue-100 text-blue-800' : 
                            rule.type === 'event' ? 'bg-purple-100 text-purple-800' : 
                            'bg-orange-100 text-orange-800'
                          }`}>
                            {rule.type === 'schedule' ? '스케줄' : rule.type === 'event' ? '이벤트' : '조건'}
                          </span>
                        </div>
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                          ...
                          <div>
                            ...
                            <span className="ml-2 font-medium">
                              {rule.lastExecuted ? new Date(rule.lastExecuted).toLocaleString() : '없음'}
                            </span>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center gap-2">
                        <button
                          ...
                          title={rule.status === 'active' ? '비활성화' : '활성화'}
                        >
          ...
          <div className="bg-white rounded-xl shadow-lg p-12 text-center">
            <div className="text-gray-500">
              ...
              <p className="text-lg font-medium mb-2">
                {activeTab === 'logs' ? '실행 로그' : 
                 activeTab === 'queue' ? '메시지 큐' : '분석'} 기능
              </p>
