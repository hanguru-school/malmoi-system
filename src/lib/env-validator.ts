...
export function validateEnvironment(): ValidationResult {
  ...
  const validations = [
    ...
    },
    {
      ...
      validator: (value: string) => value === "ap-northeast-1_ojlXfDMDm",
      message:
        "NEXT_PUBLIC_COGNITO_USER_POOL_ID must be ap-northeast-1_ojlXfDMDm",
    },
    {
    ...
    },
    {
      ...
      validator: (value: string) => value === "4bdn0n9r92huqpcs21e0th1nve",
      message:
        "NEXT_PUBLIC_COGNITO_CLIENT_ID must be 4bdn0n9r92huqpcs21e0th1nve",
    },
    {
  ...
  if (
    ...
    process.env.AUTH_BASE_URL?.includes("localhost")
  ) {
    result.warnings.push(
      "AUTH_BASE_URL should not contain localhost in production",
    );
  }

  if (
    process.env.NODE_ENV === "production" &&
    process.env.NEXT_PUBLIC_COGNITO_CALLBACK_URL?.includes("localhost")
  ) {
    result.warnings.push(
      "NEXT_PUBLIC_COGNITO_CALLBACK_URL should not contain localhost in production",
    );
  }
