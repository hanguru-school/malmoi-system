...
1. **学生詳細 → 「レッスン時間」タブ**
2. **クイック** ボタン **+600分 / +300分 / +180分** で `追加する分数` に反映（パッケージ選択はクリア）。
3. **メモ（理由）** を入力。
4. **レッスン時間のみ反映** で確定（`type: charge` の原簿・既存 PATCH 契約のまま）。

### 롤백

- クイックボタンと説明文を UI から外す（Git 戻し）。

### 운영 주의

- クイックはあくまで入力補助。**メモ必須**のルールは変えていません。
- 手動調整・減算は従来どおり別ブロックです。
