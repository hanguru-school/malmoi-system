...
function migrateStudentShape(student) {
  ...
  student.points = {
    balance: Math.max(0, Number(student?.points?.balance ?? student?.pointsBalance ?? 0)),
...
function filterStudents(students, filters) {
  ...
  return students.filter((item) => {
    if (query) {
      const haystack = [item.studentNumber, item.nameKanji, item.nameFurigana, item.email, item.linkedUserEmail]
        .filter(Boolean)
        .join(" ")
        .toLowerCase();
...
function buildGeneratedSlots(store, dateValue, mode) {
  ...
  const pushSlot = (timeMin, durationMinutes) => {
    if (timeMin + durationMinutes > endMin) return;
    slots.push({
      id: newId(),
      date: dateValue,
  ...
  if (mode === "all_times") {
    durations.forEach((duration) => {
      for (let cursor = startMin; cursor + duration <= endMin; cursor += 5) {
...
function applyLessonMinuteDeduction(store, reservation, actor = null) {
  ...
  const beforeUsed = Number(student.lessonMinutes.usedMinutes || 0);
  const beforeRemaining = Number(student.lessonMinutes.remainingMinutes || 0);
  student.lessonMinutes.usedMinutes = beforeUsed + minutes;
  student.lessonMinutes.remainingMinutes = Math.max(
    0,
    Number(student.lessonMinutes.totalMinutes || 0) - student.lessonMinutes.usedMinutes
  );
  student.lessonMinutes.updatedAt = nowIso();
...
function applyLessonMinuteRefundForCancellation(store, reservation, actor = null, policySource = "admin") {
  ...
  const beforeRemaining = Number(student.lessonMinutes.remainingMinutes || 0);
  student.lessonMinutes.usedMinutes = Math.max(
    0,
    Number(student.lessonMinutes.usedMinutes || 0) - refundMinutes
  );
  student.lessonMinutes.remainingMinutes = Math.max(
    0,
    Number(student.lessonMinutes.totalMinutes || 0) - student.lessonMinutes.usedMinutes
...
function applyStudentLessonMinuteManualAdjustment(store, student, deltaMinutes, actor = null, reason = "") {
  ...
  const nextTotal = Math.max(beforeUsed, beforeTotal + deltaMinutes);
  student.lessonMinutes.totalMinutes = nextTotal;
  student.lessonMinutes.remainingMinutes = Math.max(0, nextTotal - beforeUsed);
  student.lessonMinutes.updatedAt = nowIso();
  student.updatedAt = student.lessonMinutes.updatedAt;
...
function applyStudentLessonMinuteCredit(
  ...
  student.lessonMinutes.totalMinutes = beforeTotal + minutes;
  student.lessonMinutes.remainingMinutes = Math.max(
    0,
    Number(student.lessonMinutes.totalMinutes || 0) - Number(student.lessonMinutes.usedMinutes || 0)
  );
  student.lessonMinutes.updatedAt = nowIso();
  student.updatedAt = student.lessonMinutes.updatedAt;
...
function applyStudentLessonMinuteDebit(store, student, debitMinutes, actor = null, reason = "", transactionId = null) {
  ...
  if (dm <= 0) return;
  migrateStudentShape(student);
  const beforeRem = Number(student.lessonMinutes.remainingMinutes || 0);
  const beforeTotal = Number(student.lessonMinutes.totalMinutes || 0);
  const used = Number(student.lessonMinutes.usedMinutes || 0);
  const dec = Math.min(dm, beforeRem);
  student.lessonMinutes.totalMinutes = Math.max(used, beforeTotal - dec);
  student.lessonMinutes.remainingMinutes = Math.max(
    0,
    Number(student.lessonMinutes.totalMinutes || 0) - used
  );
  student.lessonMinutes.updatedAt = nowIso();
  student.updatedAt = student.lessonMinutes.updatedAt;
  ...
  writeAuditLog(store, {
    ...
    targetType: "student",
    targetId: student.id,
    summary: `Debit lesson minutes for adjustment ${transactionId || ""}`,
    meta: { minutes: dec, reason, transactionId },
  });
}
...
export async function requestPasswordResetByStudentIdentity({
  ...
  return enqueue(async () => {
    ...
    const student =
      store.students.find(
        (item) =>
          String(item.nameKanji || "").trim() === cleanNameKanji &&
          normalizePhone(item.phoneNormalized || item.phone || item?.crmProfile?.phoneMobile) === normalizedPhone &&
...
export async function updateSystemSettingsSectionByAdmin(section, patch = {}, actor = null) {
  return enqueue(async () => {
    ...
    const allowedSections = ["schoolBasic", "reservation", "lesson", "homework", "notifications", "mail", "security", "parent", "pair"];
...
export async function createReservationByStudent(userId, payload) {
  return enqueue(async () => {
    ...
    writeAuditLog(store, {
      ...
      meta: { studentId: student.id, status: reservation.status },
    });
...
export async function rescheduleReservationByStudent(userId, reservationId, payload) {
  return enqueue(async () => {
    ...
    writeAuditLog(store, {
      ...
      meta: { studentId: student.id, fromSlotId, toSlotId: targetSlot.id },
    });
...
export async function updateReservationByAdmin(reservationId, patch, actor = null) {
  return enqueue(async () => {
    ...
    if (
      ["attended", LESSON_MINUTES_DEDUCT_ON_NO_SHOW ? "no_show" : ""].includes(reservation.attendanceStatus) &&
      reservation.status !== "cancelled"
    ) {
