...
function studentMatchesRiskSignalFilter(badges, riskSignal) {
  ...
  if (sig === "minutes_depleted") return ids.has("minutes_depleted");
  if (sig === "minutes_low") return ids.has("minutes_low");
  if (sig === "minutes_short_next") return ids.has("minutes_short_next");
  return true;
}
...
function buildRiskBadgesForStudent(store, sid) {
  ...
  if (studentRow) {
    ...
    if (rem <= 0) {
      badges.push({ id: "minutes_depleted", label: "残り時間0以下", tone: "danger" });
    } else if (rem <= 180) {
    ...
    if (nextRes) {
      const need = Math.max(0, Number(nextRes.durationMinutes || 0));
      if (need > 0 && rem < need) {
        badges.push({ id: "minutes_short_next", label: "次回予約で時間不足の恐れ", tone: "warn" });
      }
    }
  }
