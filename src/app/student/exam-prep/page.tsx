...
interface Test {
  ...
  title: string;
  description: string;
  category: 'vocabulary' | 'grammar' | 'writing' | 'listening' | 'speaking' | 'reading';
        ...
        const mockTests: Test[] = [
          {
            id: '1',
            title: '기초 어휘 테스트',
            description: '일상생활에서 자주 사용하는 기본 어휘들을 테스트합니다.',
            category: 'vocabulary',
            difficulty: 'easy',
            duration: 15,
            questionCount: 20,
        ...
        const mockResults: TestResult[] = [
          {
            ...
            recommendations: ['일상 어휘는 잘 알고 있습니다. 비즈니스 어휘를 더 학습해보세요.']
          },
          {
            id: '2',
            testId: '5',
            ...
            accuracy: 78,
            weakAreas: ['자음 발음', '억양'],
            recommendations: ['자음 발음 연습이 필요합니다. 특히 ㅅ, ㅈ, ㅊ 발음을 더 연습해보세요.']
          }
        ];
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50 p-6">
      <div className="max-w-6xl mx-auto">
        {/* 헤더 */}
        <div className="mb-8">
          <h1 className="text-4xl font-bold text-gray-900 mb-2">자가 진단 테스트</h1>
          ...
          <div className="bg-white rounded-xl shadow-lg p-6 mb-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-6">
              <div className="bg-blue-50 rounded-lg p-4">
                ...
                <div className="text-2xl font-bold text-blue-600">{stats.totalTests}개</div>
              </div>
              ...
              <div className="bg-green-50 rounded-lg p-4">
                ...
                <div className="text-2xl font-bold text-green-600">{stats.completedTests}개</div>
              </div>
              ...
              <div className="bg-purple-50 rounded-lg p-4">
                ...
                <div className="text-2xl font-bold text-purple-600">{stats.averageScore}점</div>
              </div>
              ...
              <div className="bg-orange-50 rounded-lg p-4">
                ...
                <div className="text-2xl font-bold text-orange-600">{stats.totalStudyTime}분</div>
              </div>
            </div>

            {/* 영역별 성과 */}
            <div className="bg-gradient-to-r from-indigo-50 to-purple-50 rounded-lg p-4">
              <h3 className="text-lg font-semibold text-gray-900 mb-3">영역별 성과</h3>
              <div className="grid grid-cols-2 md:grid-cols-6 gap-4">
                {Object.entries(stats.categoryPerformance).map(([category, score]) => (
                  <div key={category} className="text-center">
                    ...
                    <div className="text-lg font-bold text-gray-900 mb-1">{score}점</div>
                    <div className="text-xs text-gray-600">{getCategoryText(category)}</div>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {filteredTests.map((test) => (
                <div key={test.id} className="bg-white rounded-xl shadow-lg p-6 hover:shadow-xl transition-shadow">
                  ...
                  <p className="text-gray-700 mb-4 line-clamp-2">{test.description}</p>
                  
                  {/* 테스트 정보 */}
                  <div className="grid grid-cols-2 gap-4 mb-4 text-sm">
                    <div>
                      <span className="text-gray-600">문제 수:</span>
                      <span className="ml-2 font-medium">{test.questionCount}개</span>
                    </div>
                    <div>
                      <span className="text-gray-600">소요시간:</span>
                      <span className="ml-2 font-medium">{test.duration}분</span>
                    </div>
                  </div>
                  ...
                  {test.score !== undefined && (
                    <div className="mb-4 p-3 bg-blue-50 rounded-lg">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm font-medium text-blue-800">점수: {test.score}점</span>
                      </div>
                    </div>
                  )}
                ...
                <div key={result.id} className="border border-gray-200 rounded-lg p-4">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-3">
                    <div>
                      ...
                      <div className="text-lg font-bold text-blue-600">{result.score}/{result.maxScore}점</div>
                    </div>
                    <div>
                    ...
                    <div>
                      ...
                      <div className="text-lg font-bold text-orange-600">{result.timeSpent}분</div>
                    </div>
                    <div>
                  ...
                  {result.weakAreas.length > 0 && (
                    <div className="mb-3">
                      <span className="text-sm font-medium text-gray-700">개선이 필요한 영역:</span>
                  ...
                  {result.recommendations.length > 0 && (
                    <div>
                      <span className="text-sm font-medium text-gray-700">추천사항:</span>
                      <div className="mt-1">
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl p-6 max-w-2xl w-full mx-4 max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-2xl font-semibold text-gray-900">{selectedTest.title}</h3>
              <button
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <h4 className="font-medium text-gray-900 mb-2">테스트 정보</h4>
                  <div className="space-y-2 text-sm">
                    <div>
                      <span className="text-gray-600">영역:</span>
                      <span className="ml-2">{getCategoryText(selectedTest.category)}</span>
                    </div>
                    <div>
                      <span className="text-gray-600">난이도:</span>
                      <span className="ml-2">{selectedTest.difficulty === 'easy' ? '쉬움' : 
                       selectedTest.difficulty === 'medium' ? '보통' : '어려움'}</span>
                    </div>
                    <div>
                      <span className="text-gray-600">문제 수:</span>
                      <span className="ml-2">{selectedTest.questionCount}개</span>
                    </div>
                    <div>
              ...
              {selectedTest.score !== undefined && (
                <div className="bg-blue-50 rounded-lg p-4">
                  <h4 className="font-medium text-blue-900 mb-2">테스트 결과</h4>
                  <div className="text-2xl font-bold text-blue-600">{selectedTest.score}점</div>
                  <p className="text-sm text-blue-700 mt-1">
                    완료일: {selectedTest.completedAt && new Date(selectedTest.completedAt).toLocaleDateString('ko-KR')}
                  </p>
                </div>
              )}
