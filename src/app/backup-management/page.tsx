...
export default function BackupManagementPage() {
  ...
  const [restoreType, setRestoreType] = useState<'history' | 'rolling' | 'protected'>('rolling');
  const [showSettings, setShowSettings] = useState(false);
  const [backupConfig, setBackupConfig] = useState<Record<string, unknown>>(null);
  ...
  const handleRestoreBackup = async () => {
    ...
    if (!confirm(`정말로 ${selectedBackup.version} (${selectedBackup.type}) 백업을 복원하시겠습니까?`)) {
      return;
    }

    try {
      setLoading(true);
      const response = await fetch(`/api/backups/${selectedBackup.version}/restore`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const handleRemoveFromProtected = async (version: string) => {
    ...
    try {
      setLoading(true);
      const response = await fetch(`/api/backups/protected?version=${version}`, {
        method: 'DELETE',
      });
  ...
  const handleDownloadBackup = async (backup: BackupInfo) => {
    try {
      ...
      // 백업 정보 확인
      const infoResponse = await fetch(`/api/backups/download?version=${backup.version}&type=${backup.type}`);
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50 p-6">
      <div className="max-w-7xl mx-auto">
            ...
            <div className="bg-white rounded-xl shadow-lg p-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <div className="text-2xl font-bold text-gray-900">{stats.historyCount}</div>
                </div>
            ...
            <div className="bg-white rounded-xl shadow-lg p-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <div className="text-2xl font-bold text-gray-900">{stats.protectedCount}/5</div>
                </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="bg-white rounded-xl shadow-lg p-6">
            ...
            {loading ? (
              ...
              <div className="space-y-2">
                {backups.history.slice(-5).reverse().map((backup) => (
                  <div
                    key={backup.version}
                  ...
                  >
                    <div className="flex items-center justify-between mb-2">
                      <div>
                        <div className="font-medium text-gray-900">{backup.version}</div>
                        <div className="text-sm text-gray-600">
                          {new Date(backup.timestamp).toLocaleDateString()}
                        </div>
                      </div>
                      <div className="text-right">
                        <div className="text-sm text-gray-600">{formatFileSize(backup.size)}</div>
                      </div>
                    </div>
          ...
          <div className="bg-white rounded-xl shadow-lg p-6">
            ...
            {loading ? (
              ...
              <div className="space-y-2">
                {backups.rolling.slice(-5).reverse().map((backup) => (
                  <div
                    key={backup.version}
                    className="p-3 border border-gray-200 rounded-lg hover:bg-gray-50"
                  >
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <div className="p-3 bg-gray-50 rounded-lg">
                    ...
                    <div className="text-sm text-gray-600">{selectedBackup.type}</div>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <select
                    value={restoreType}
                    onChange={(e) => setRestoreType(e.target.value as 'history' | 'rolling' | 'protected')}
        ...
        {showProtectedList && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white rounded-xl p-6 max-w-md mx-4">
              <h3 className="text-lg font-semibold text-gray-900 mb-4">보호 목록 관리</h3>
              <div className="space-y-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">
                    보호 목록에 추가
                  ...
                  <>
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => setBackupConfig({...backupConfig, basePath: e.target.value})}
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => setBackupConfig({...backupConfig, maxRollingBackups: parseInt(e.target.value)})}
                        className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent"
                      />
                      <p className="text-xs text-gray-500 mt-1">
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => setBackupConfig({...backupConfig, maxProtectedBackups: parseInt(e.target.value)})}
                        className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent"
                      />
                      <p className="text-xs text-gray-500 mt-1">
                    ...
                    <div>
                      ...
                      <div className="flex gap-2">
                        ...
                        <button
                          onClick={() => {
                            ...
                            alert('브라우저 보안상 폴더 선택이 제한됩니다. 경로를 수동으로 입력해주세요.');
                          }}
                      ...
                      <p className="text-xs text-gray-500 mt-1">
                        백업 다운로드 시 기본 저장 경로 (예: ~/Downloads/backups)
                      </p>
                    </div>
                  </>
                )}
