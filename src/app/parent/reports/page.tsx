...
const mockReports = [
  {
    ...
    teacherComment: '민수는 수학에 대한 이해도가 높아지고 있습니다. 특히 분수 개념을 잘 이해하고 있어요. 계속해서 꾸준한 학습을 유지해주세요.',
  ...
  },
  {
    ...
    teacherComment: '영어 발음이 많이 좋아졌습니다. 하지만 문법 부분에서 아직 부족한 점이 있어요. 더 많은 연습이 필요합니다.',
  ...
  },
  {
    ...
    teacherComment: '과학 실험에 대한 관심이 높고, 호기심이 많아서 좋습니다. 관찰력이 뛰어나고 기록도 잘 하고 있어요.',
...
export default function LearningReportsPage() {
  ...
  const filteredReports = mockReports.filter(report => {
    const matchesSubject = selectedSubject === '전체' || report.subject === selectedSubject;
  ...
  const averageScore = filteredReports.length > 0 
    ? filteredReports.reduce((acc, report) => acc + report.score, 0) / filteredReports.length 
    : 0;
        ...
        <div className="bg-white p-6 rounded-lg shadow-sm border">
          <div className="flex items-center">
            ...
            <div className="ml-4">
              ...
              <p className="text-2xl font-bold text-gray-900">{averageScore.toFixed(1)}점</p>
            </div>
          </div>
        </div>
        <div className="bg-white p-6 rounded-lg shadow-sm border">
        ...
        {filteredReports.map((report) => (
          <div key={report.id} className="bg-white p-6 rounded-lg shadow-sm border">
            <div className="flex justify-between items-start mb-6">
              <div>
                <h3 className="text-xl font-semibold text-gray-900">{report.subject}</h3>
                <p className="text-sm text-gray-600">{report.period} • {report.childName}</p>
              </div>
              <div className="text-right">
                <div className={`text-3xl font-bold ${getGradeColor(report.overallGrade)}`}>
                  {report.overallGrade}
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
              <div className="bg-gray-50 p-4 rounded-lg">
                <div className="flex items-center justify-between">
                  <span className="text-sm font-medium text-gray-600">이전 점수</span>
                  <span className="text-lg font-semibold text-gray-900">{report.previousScore}점</span>
                </div>
              </div>
              <div className="bg-gray-50 p-4 rounded-lg">
                <div className="flex items-center justify-between">
                  <span className="text-sm font-medium text-gray-600">향상도</span>
                  <span className={`text-lg font-semibold ${getImprovementColor(report.improvement)}`}>
                    {report.improvement}
                  </span>
                </div>
              </div>
              <div className="bg-gray-50 p-4 rounded-lg">
                <div className="flex items-center justify-between">
                  <span className="text-sm font-medium text-gray-600">출석률</span>
                  <span className="text-lg font-semibold text-gray-900">{report.attendance}%</span>
                </div>
              </div>
              <div className="bg-gray-50 p-4 rounded-lg">
                <div className="flex items-center justify-between">
                  <span className="text-sm font-medium text-gray-600">과제 완성도</span>
                  <span className="text-lg font-semibold text-gray-900">{report.homeworkCompletion}%</span>
                </div>
              </div>
            </div>
            ...
            {/* 선생님 코멘트 */}
            <div className="mb-6">
              <h4 className="text-lg font-semibold text-gray-900 mb-2">선생님 코멘트</h4>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
              <div>
                <h4 className="text-lg font-semibold text-gray-900 mb-3">강점</h4>
              ...
              <div>
                <h4 className="text-lg font-semibold text-gray-900 mb-3">개선이 필요한 부분</h4>
                <div className="space-y-2">
            ...
            {/* 다음 목표 */}
            <div>
              <h4 className="text-lg font-semibold text-gray-900 mb-3">다음 목표</h4>
              <div className="space-y-2">
